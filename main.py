...
def voice_processing(message):
    if message:
        # Send initial waiting message and store its message ID
        waiting_message = bot.reply_to(message, 'Подождите немного, я обрабатываю голосовое сообщение :)')
        ...
        try:
            ...
            # Replace initial waiting message with first chunk result
                ...
                try:
                    with sr.AudioFile(chunk_path) as source:
                        ...
                        if not first_chunk_processed:
                            # Replace waiting message with first chunk
                    ...
                    if not first_chunk_processed:
                        # Replace waiting message with error for first chunk
                        bot.edit_message_text(
                            chat_id=message.chat.id, 
                            message_id=waiting_message.message_id, 
                            text=f'Часть {i}: не распознана'
                        )
                        first_chunk_processed = True
                    else:
                        # Send error for subsequent chunks
                        bot.send_message(message.chat.id, f'Часть {i}: не распознана')
                    ...
                    if not first_chunk_processed:
                        ...
                        bot.edit_message_text(
                            chat_id=message.chat.id, 
                            message_id=waiting_message.message_id, 
                            text=f'Часть {i}: ошибка распознавания'
                        )
                        first_chunk_processed = True
                    else:
                        # Send error for subsequent chunks
                        bot.send_message(message.chat.id, f'Часть {i}: ошибка распознавания')
            ...
            # If initial processing fails completely
            bot.edit_message_text(
                chat_id=message.chat.id, 
                message_id=waiting_message.message_id, 
                text=f'Извините, произошла ошибка при распознавании: {str(e)}'
            )
        finally:
